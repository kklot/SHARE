...
here::i_am("code/NHV_model_up_female.r")
...
# md.ref = readRDS("/scratch/fuchs/fias/knguyen/SHARE/fitted/md.male.free.rds")
md.ref <- readRDS("/scratch/fuchs/fias/knguyen/SHARE/fitted/md.female.free.rds")

md.female <- inla(f0,
    E = log_pm, data = female.data,
    family = c("poisson", "cenpoisson", "poisson"),
    control.family = list(list(), list(cenpoisson.I = NHV.interval), list()),
    control.compute = list(config = TRUE),
    control.inla = list(int.strategy = "eb"),
    control.mode = list(theta = md.ref$mode$theta, restart = TRUE),
    control.predictor = list(link = 1),
    verbose = FALSE
)

saveRDS(md.female, "/scratch/fuchs/fias/knguyen/SHARE/fitted/md.female.free.update2.rds")
